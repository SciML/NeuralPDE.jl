...
## Features
  ...
  - PDE discretization to a `ModelingToolkit.System` whose optimization problem is generated by the
    standard ModelingToolkit compiler: weighted or constrained boundary conditions, quadrature and
    quasi-random training strategies, and Reactant-compiled training loops
  - Handling of various stochastic equations
...
res = solve(prob, Adam(0.1); callback = callback, maxiters = 4000)
prob = remake(prob, u0 = res.original_sol.u)
sol = solve(prob, Adam(0.01); callback = callback, maxiters = 2000)
```
