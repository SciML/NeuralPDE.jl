...
sol = solve(prob, NeuralPDE.NNODE(chain,opt), abstol=1f-6,
            verbose = true, maxiters=200)
...
sol  = solve(prob,NeuralPDE.NNODE(chain,opt),verbose = true, maxiters=400, dt = 1/5f0)
@test sol.errors[:l2] < 0.5

sol  = solve(prob,NeuralPDE.NNODE(chain,opt;strategy = StochasticTraining(100)),verbose = true, maxiters=400, dt = 1/5f0)
@test sol.errors[:l2] < 0.5
...
chain = Flux.Chain(Dense(1,5,σ),Dense(5,1))
opt = ADAM(0.1)
sol  = solve(prob,NeuralPDE.NNODE(chain,opt),verbose = true, maxiters=400, abstol=1f-8)
@test sol.errors[:l2] < 0.5

sol  = solve(prob,NeuralPDE.NNODE(chain,opt),verbose = true, maxiters=400, abstol=1f-8, dt = 1/5f0)
@test sol.errors[:l2] < 0.5

sol  = solve(prob,NeuralPDE.NNODE(chain,opt;strategy = StochasticTraining(100)),verbose = true, maxiters=400, abstol=1f-8, dt = 1/5f0)
@test sol.errors[:l2] < 0.5