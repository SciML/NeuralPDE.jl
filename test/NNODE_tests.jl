...
sol = solve(prob, NeuralPDE.NNDE(chain,opt), dt=1/20f0, verbose = true,
...
chain = Flux.Chain(Dense(1,5,σ),Dense(5,1))
opt = BFGS()
sol = solve(prob, NeuralPDE.NNDE(chain,opt), dt=1/20f0, abstol=1e-10,
...
opt = ADAM(0.01)
sol  = solve(prob,NeuralPDE.NNDE(chain,opt),verbose = true, dt=1/5f0, maxiters=200)
@test sol.errors[:l2] < 0.5

#=
dts = 1f0 ./ 2f0 .^ (6:-1:2)
sim = test_convergence(dts, prob, NeuralPDE.NNDE(chain, opt))
...
chain = Flux.Chain(Dense(1,5,σ),Dense(5,1))
opt = ADAM(0.01)
sol  = solve(prob,NeuralPDE.NNDE(chain,opt),verbose = true, dt=1/5f0)
@test sol.errors[:l2] < 0.5
