...
@testset "BPINN PDE Inv III: Improved Parametric Kuromo-Sivashinsky Equation solve" begin
    ...
    u_predict_new = [
        [mean_predict(chain, sol_new.estimated_nn_params[1], x, t) for x in xs]
            for t in ts
    ]

    diff_u_new = [
        [
            abs(u_analytic(x, t) - mean_predict(chain, sol_new.estimated_nn_params[1], x, t))
                for x in xs
        ]
            for t in ts
    ]
    ...
    u_predict_old = [
        [mean_predict(chain, sol_old.estimated_nn_params[1], x, t) for x in xs]
            for t in ts
    ]
    diff_u_old = [
        [
            abs(u_analytic(x, t) - mean_predict(chain, sol_old.estimated_nn_params[1], x, t))
                for x in xs
        ]
            for t in ts
    ]
