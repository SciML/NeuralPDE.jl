...
@test mean(abs2.(x̂ .- meanscurve1)) < 1e-3
@test mean(abs2.(physsol1 .- meanscurve1)) < 1e-3
@test mean(abs2.(x̂ .- meanscurve2)) < 2e-3
@test mean(abs2.(physsol1 .- meanscurve2)) < 2e-3

# ESTIMATED ODE PARAMETERS (NN1 AND NN2)
@test abs(p - mean([fhsamples2[i][23] for i in 1000:2000])) < 0.05 * p
@test abs(p - mean([fhsamples1[i][23] for i in 1000:2000])) < 0.05 * p
...
chainflux1 = Flux.Chain(Flux.Dense(1, 6, tanh), Flux.Dense(6, 6, tanh), Flux.Dense(6, 1))
chainlux1 = Lux.Chain(Lux.Dense(1, 6, tanh), Lux.Dense(6, 6, tanh), Lux.Dense(6, 1))
...
out = re1.([fhsamplesflux1[i][1:61] for i in 1000:1500])
yu = [out[i](t') for i in eachindex(out)]
fluxmean = [mean(vcat(yu...)[:, i]) for i in eachindex(t)]
meanscurve1_1 = prob.u0 .+ (t .- prob.tspan[1]) .* fluxmean

@test mean(abs2.(sol.u .- meanscurve1_1)) < 5e-5
@test mean(abs2.(physsol1 .- meanscurve1_1)) < 5e-5

out = re1.([fhsamplesflux2[i][1:61] for i in 1000:1500])
yu = [out[i](t') for i in eachindex(out)]
fluxmean = [mean(vcat(yu...)[:, i]) for i in eachindex(t)]
meanscurve1_2 = prob.u0 .+ (t .- prob.tspan[1]) .* fluxmean

@test mean(abs2.(sol.u .- meanscurve1_2)) < 5e-4
@test mean(abs2.(physsol1 .- meanscurve1_2)) < 5e-4

# estimated parameters(flux chain)
param1 = mean(i[62] for i in fhsamplesflux2[1000:1500])
param2 = mean(i[63] for i in fhsamplesflux2[1000:1500])
...
θ = [vector_to_parameters(fhsampleslux1[i], θinit) for i in 1000:1500]
luxar = [chainlux1(t', θ[i], st)[1] for i in 1:500]
luxmean = [mean(vcat(luxar...)[:, i]) for i in eachindex(t)]
meanscurve2_1 = prob.u0 .+ (t .- prob.tspan[1]) .* luxmean

@test mean(abs2.(sol.u .- meanscurve2_1)) < 7e-4
@test mean(abs2.(physsol1 .- meanscurve2_1)) < 7e-4

θ = [vector_to_parameters(fhsampleslux2[i][1:(end - 2)], θinit) for i in 1000:1500]
luxar = [chainlux1(t', θ[i], st)[1] for i in 1:500]
luxmean = [mean(vcat(luxar...)[:, i]) for i in eachindex(t)]
meanscurve2_2 = prob.u0 .+ (t .- prob.tspan[1]) .* luxmean

@test mean(abs2.(sol.u .- meanscurve2_2)) < 5e-4
@test mean(abs2.(physsol1 .- meanscurve2_2)) < 5e-4

# estimated parameters(lux chain)
param1 = mean(i[62] for i in fhsampleslux2[1000:1500])
param2 = mean(i[63] for i in fhsampleslux2[1000:1500])
@test abs(param1 - p[1]) < abs(0.3 * p[1])
@test abs(param2 - p[2]) < abs(0.3 * p[2])
