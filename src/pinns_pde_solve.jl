        ...
        if !(e isa Expr)
            if e isa Symbolics.Integral
                ...
                if -Inf in lb || Inf in ub
                    ...
                    ex.args[i+1] = transform_inf_expr(ex.args[i+1], integrating_variable,transform_indvars)
                end

                break
            end
        else
            ex.args[i] = transform_infinite_integral(ex.args[i])
        end
    end
...
function build_symbolic_equation(eq,_indvars,_depvars,chain,eltypeθ,strategy,phi,derivative, integral,initθ)
...
@parameters x y
@variables u(..)
I = Integral((x,y) in DomainSets.ProductDomain(ClosedInterval(-Inf ,Inf),ClosedInterval(-Inf ,Inf)))
...
depvars = [u(x, y)]
indvars = [x, y]
eqs = I(u(x, y)*x) * Dx(u(x)) ~ 0
eltypeθ = Float64
phi = NeuralPDE.get_phi(chain,eltypeθ)
derivative = NeuralPDE.get_numeric_derivative()
integral = NeuralPDE.get_numeric_integral(strategy, indvars, depvars, chain, derivative)

# equation-version
args = []
if !(eqs.rhs isa Number || eqs.rhs isa Symbol)
    append!(args, arguments(eqs.rhs))
end
if !(eqs.lhs isa Number || eqs.lhs isa Symbol)
    append!(args, arguments(eqs.lhs))
end
args

for arg in args
    if typeof(operation(arg)) == Integral{Symbolics.VarDomainPairing}
        lb, ub = get_limits(operation(arg).domain.domain)
...
#
integrating_variables = [x, y]
if first(arguments(first(arguments(args[2])))) == [x]
    @show true
end

first(arguments((args[2])))

for intvar in integrating_variables
...
function build_symbolic_loss_function(eqs,indvars,depvars,
    ...
    if strategy isa QuadratureTraining
        ...
        indvars_ex = get_indvars_ex(this_eq_indvars)
        left_arg_pairs, right_arg_pairs = this_eq_indvars, indvars_ex
        vars_eq = Expr(:(=), build_expr(:tuple, left_arg_pairs), build_expr(:tuple, right_arg_pairs))
    end
