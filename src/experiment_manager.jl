...
struct ExperimentManager{H <: AbstractHyperParameter, C, PlotFunction}
    ...
    SciMLBase.@add_kwonly function ExperimentManager(pde_system::PDESystem, hyperparameters::Vector{H}, 
        ...
        for hyperparameter in hyperparameters
            push!(hyperparameter_queue, hyperparameter)
        end
...
function run_experiment_queue(experiment_manager::ExperimentManager{H}; remote=true, startat=1, endat=length(experiment_manager.hyperparameter_queue), distributed=true) where {H <: AbstractHyperParameter}
    ...
    num_experiments = length(hyperparameter_queue)
    for i in 1:startat-1
        popfirst!(hyperparameter_queue)
    end
    for i in endat:num_experiments - 1
        pop!(hyperparameter_queue)
    end
    num_experiments = length(hyperparameter_queue)
    println("executing $(num_experiments) experiments")
    ...
    if distributed
        while true
            ...
            for worker_index in 1:num_workers
                ...
                if experiments_in_progress[worker_index] isa Nothing && length(hyperparameter_queue) > 0
                    ...
                    hyperparam = popfirst!(hyperparameter_queue)
        ...
        for i in 1:num_experiments
            ...
            # change experiment_in_progress data structure and make a new future to store results in
            #experiments_in_progress[worker_index] = ExperimentInProgress{H}(hyperparam, Future(myid()))
            println("assigned experiment $experiment_index to serial queue")
            println("$(length(hyperparameter_queue)) hyperparameters left in queue")
            NeuralPDE.remote_run_neuralpde_with_logs(pde_system, hyperparam, cb_func, log_options, experiment_index, remote)
            experiment_index += 1
        end
    end
