...
function NNKolmogorov(chain; opt = Flux.ADAM(0.1), sdealg = EM(),
                      ensemblealg = EnsembleThreads())
    NNKolmogorov(chain, opt, sdealg, ensemblealg)
end

function DiffEqBase.solve(prob::Union{KolmogorovPDEProblem, SDEProblem},
                          alg::NNKolmogorov;
                          abstol = 1.0f-6,
                          verbose = false,
                          maxiters = 300,
    ...
    function prob_func(prob, i, repeat)
        SDEProblem(prob.f, prob.g, xi[:, i], prob.tspan,
                   noise_rate_prototype = prob.noise_rate_prototype)
    end
