...
function NNStopping(chain; opt = Flux.ADAM(0.1), sdealg = EM(),
                    ensemblealg = EnsembleThreads())
    NNStopping(chain, opt, sdealg, ensemblealg)
end
    ...
    function Un(n, X)
        if size(un)[1] >= n
            ...
            if (n == 1)
                ...
                ans = max(first(m(X[n])[n]), n + 1 - size(ts)[1]) *
                      (1 - sum(Un(i, X) for i in 1:(n - 1)))
                un = vcat(un, ans)
                return ans
            end
        end
