...
minimizers_ = [res.u[Symbol("θ",i)] for i in 1:3]
...
is from Lux.jl, it's given as a `ComponentArray` where `res.u.θi` corresponds to the result
for the ith neural network, i.e. `res.u.θ1` are the trained parameters for `phi[1]`. For
simpler indexing, you can use `res.u[:θ1]` or `res.u[Symbol("θ",1)]` as shown here. Subsetting
the array also works, but is inelegant.
