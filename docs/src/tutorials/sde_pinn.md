# SDE-PINNs for solving a Geometric Brownian Motion Stochastic ODE.

This tutorial walks through using `NeuralPDE's` unique SDE-PINN solver for forward problems, specifically the Geometric Brownian Motion (GBM) Stochastic Differential Equation (SDE).

GBM also called exponential brownian motion, is a continous time stochastic process in which the logarithm of the randomly varying(diffusion) quantity follows a Wiener process with drift. A Stochastic Process $`S_t`$ following GBM satisfies the following SDE:
...
\end{align*}
```
Where, $`W_t`$ is a Wiener Process (Brownian Motion), `μ` is the drift parameter (used to model the deterministic part of motion) and `σ` is the diffusion controlling parameter (used to model the stochastic part of motion).

For our system we set `μ`= 0.5 and `σ` = 1.0.
...
```@example gbm_sde
dim = 1 + 3
luxchain = Chain(Dense(dim, 16, Lux.σ), Dense(16, 16, Lux.σ), Dense(16, 1))
```
...
analytic_sol(u0, p, t, W) = u0 * exp((μ - σ^2 / 2) * t + σ * W)
...
function truncated_sol(u0, t, z1, z2, z3)
    u0 *
    exp((μ - σ^2 / 2) * t + σ * W_kkl(t, z1, z2, z3))
end
